...
export const MAIN_SUB_APP_LIST = [...CREATE_DATA_LIST, ...CREATE_BUSINESS_LIST, ...CREATE_VIRTUAL_LIST];
// 数据资产
...
export const HELP_LIST = [
  {
    name: "平台说明",
    desc: "平台主要由登录页、构建端和运行端组成，其中构建端由项目页、应用页、表单构建页、流程构建页、权限配置页、扩展设置页组成；运行端由任务中心页、流程数据管理页、流程详情页、任务详情页等组成",
  },
  {
    name: "表单编排",
    desc: "表单编排页面包括展示内容包括三个部分，分别左侧为控件展示区域、中间为画布区域、右侧为属性及规则配置区域",
  },
  {
    name: "流程编排",
    desc: "流程编排页面包括展示内容包括两个部分，分别左侧为画布区域、右侧为节点属性配置区域",
  },
  {
    name: "任务中心",
  ...
  {
    name: "接口编排",
    desc: "自定义服务积木，使用第三方API服务，该API服务未导入到系统中，该积木会根据指定的Url、请求类型及请求参数，调用该API，返回调用该API的响应结果",
  },
  {
    name: "报表编排",
    desc: "报表编排包括仪表盘、统计图表、数据接入、数据规则共四个模块，顶部提供各类统计图表，中间画布支持自由布局，点击图表进行数据源选择及规则配置",
  },
  {
    name: "大屏编排",
    desc: "画布编辑、发布和数据源对接能力，需进行页面名称填写并设置分组，同时可以选择某个模板作为当前的编辑页面或者选择空白页作为当前页面",
  },
  {
    name: "接入数据",
    desc: "模型组件用于对左侧源数据进行数据治理，通过组件自由组合实现，数据的灵活治理，包括结果输出、单表操作、多表操作",
  },
];
